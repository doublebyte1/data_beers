...
%\begin{itemize}    
%      \item<1->This (\textit{Big}?) amount of data is not easily assimilated by the "human-eye".
%\end{itemize}          
%  \begin{figure}   
...
\begin{columns}
  \begin{column}{0.5\textwidth}
      ...
      %\begin{itemize}    
	    ...
	    %\item<1->Data mining/ Machine Learning      
      %\end{itemize}                      
  \end{column}
  \begin{column}{0.5\textwidth}      
	\begin{figure}   
...
\begin{columns}
  \begin{column}{0.5\textwidth}\small{ 
    \begin{itemize}    
	  %\item<1->Detects an a priori unknown, number of clusters with an arbitrary shape. %It implements a strict policy regarding what a
	  \item<1->Density-based clustering.% algorithm that 
  ...
  \begin{column}{0.5\textwidth}    
        %\begin{figure}   
	%\includegraphics[width=0.7\textwidth]{epsilon_minpts.png}   
      %\end{figure}     
      \begin{figure}   
...
%\begin{itemize}
%      \item<1->Larger epsilon, lower minpts: larger clusters that include a great part of the points, but it fails to expose the details of the highly dense zones in the city centre.
%      \item<1->Lower epsilon, larger minpts: detects these highly dense zones, but leaves out other \textit{potential} clusters that are less dense, in the city outskirts.
%\end{itemize}                
%\end{frame}
...
\begin{itemize}%It is considered a    
      \item<1->Generalization of DBSCAN.%, tackling its difficulty in configuring the parameters.%It is essentially an  
...
%This is represented in a diagram plotting this ordering and a special distance that is stored for each point, representing the density that needs to be accepted for a cluster in order to have both points belong to the same cluster.      
      \item<1->It does \textbf{not} produce a strict partition of the data.%, but it is possible to generate a clustering structure from the OPTICS output using another algorithm.     
\end{itemize}                
%  \begin{figure}   
...
\begin{itemize}
      ...
      \item<1->We can define the \textit{start} and \textit{end} of a cluster, based on a threshold (xi).%Since "outer" clusters contain clusters with smaller reachability distances, The results of this partition will be hierarchical
      \item<1->Hierarchical partition.%more difficult to interpret, but it reveals something about teir structure.
\end{itemize}                
  \begin{figure}   
...
\frametitle{NASA WW}
    \begin{itemize}
      \item<1->WorldWind is an Open Source virtual globe developed by NASA that accesses a number of geographic datasets.%, including the Blue Marble Next Generation imagery, apart from other georeferenced images and geographic features served over the internet 
      \begin{itemize}
	  \item<1->OpenGL.%hardware accelerated rendering
       \end{itemize}
      \end{itemize}
...
\begin{columns}
  \begin{column}{0.4\textwidth}
    \begin{itemize}
      ...
      \item<1->Generates clusters based on DBSCAN, OPTICSxi (or both). 
      \item<1->Displays the results on a virtual globe.
      \end{itemize}      
  \end{column}
...
\frametitle{Conclusions}
    \begin{itemize}    
      ...
      \item<1->Hierarchical cluster partition: allows to look at the city through multiple scales.%Descending at the detail we want 
      ...
      %\item<1->The reproducibility of this approach is encouraged by the use of FOSS technologies.      
     \end{itemize}
